...
\KwOut{$Y$: Dataset with reduced dimensions}
\bigskip
\For{ $x \in X$}{
    $SS \leftarrow \bigcup_{x \in X}$localFuzzySimplicialSet($X, x, n$)
}
...
\KwOut{fs-set: output fuzzy simplicial set}
\bigskip
...
fs-set$_{1} \leftarrow$ fs-set$_{1} \bigcup$ $([x,y], \exp (-d_{x,y}))$
}
\Return fs-set
...
\SetAlgoLined
\KwIn{
knn-dists: distances of k-nearest neighbors\\
$n$: the neighborhood size to used for local metric approximation\\
$\rho$: Distance to nearest neighbor\\
}
\KwOut{$\sigma$: normalizing factor for distances}
\bigskip
binSearch(knn-dists) such that $\Sigma^{n}_{i=1} \exp (-($knn-dists$_{i} - \rho) / \sigma) = \log_{2}(n)$
\Return $\sigma$
\caption{smoothKNNDist}
\label{alg:smoothKNNDist}
\end{algorithm}

\begin{algorithm}[H]
\SetAlgoLined
\KwIn{
top-rep: Fuzzy simplicial set representation of the data\\
$d$: the neighborhood size to used for local metric approximation\\
}
\KwOut{$Y$: Spectral embedding}
\bigskip
...
$Y \leftarrow $evec$[1\dots d + 1]$\\
\Return $Y$
\caption{spectralEmbedding}
\label{alg:spectralEmbedding}
\end{algorithm}
...
$min-dist$: The minimum distance between points in the embedding\\
$n-epochs$: number of iterations used for optimization\\
}
\KwOut{$Y$: optimized embedding}
\bigskip
$\alpha \leftarrow 1.0$\\
Fit $\phi$ from $\psi$ defined by min-dist\\
\For{ $e \leftarrow 1, \dots ,$n-epochs }{
    \For{$([a,b],p) \in$ top-rep_{1} }{
        \If{Random() \leq $p$}{
            $y_a \leftarrow y_a + \alpha * \delta (\log (\phi))(y_a, y_b)$\\
            \For{$i \leftarrow 1, \dots,$n-neg-samples}{
                $c \leftarrow$ random sample from Y\\
                $y_a \leftarrow y_a + \alpha * \delta (\log (\phi))(y_a, y_c)$
            }
        }{}
    }
...
\Return $Y$
\caption{Optimizing the embedding}
\label{alg:optimizeEmbedding}
\end{algorithm}
